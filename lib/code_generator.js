...
{
    var compiler = scope.compiler,
        indentString = compiler.format.valueForProperty(null, scope, "*", "indent-string"),
        indentWidth = compiler.format.valueForProperty(null, scope, "*", "indent-width");
...
ElseIfStatement: function(node, scope, compileNode)
{
    ...
    generators.IfStatement(node, scope, compileNode);
    /*eslint-enable */
},

ElseStatement: function(node, scope, compileNode)
{
...
Lambda: function(node, scope, compileNode)
{
    /*eslint-disable new-cap */
    generators.BlockStatement(node, scope, compileNode);
    /*eslint-enable */
},
...
{
    var buffer = scope.compiler.jsBuffer,
        singleLineLimit = scope.compiler.format.valueForProperty(null, scope, "*", "single-line-array-limit"),
...
ProtocolLiteralExpression: function(node, scope)
{
    var buffer = scope.compiler.jsBuffer,
        name = node.id.name,
