...
function checkParameterTypes(compiler, node, nodeArguments, types, returnType, overriddenMethod, selector)
{
    ...
    for (var i = 1; i < declaredTypes.length; i++)
    {
            ...
            !(parameterType === "id" && nodeArguments[i - 1].type.typeisclass))
        {
            ...
            // To avoid complaint from WebStorm inspection
...
function checkForShadowedVars(node, scope)
{
    ...
    if (variable)
    {
        ...
        // An implicit global here means a var declaration
    ...
    else
    {
        ...
        else if (compiler.isPredefinedGlobal(identifier))
        {
...
function checkIdentifierReference(node, scope)
{
    ...
    if (scope.assignment)
    {
        // Check for assignment to read-only predefined global
        ...
        else if (scope.isLocalScope() && compiler.shouldWarnAbout("implicit-globals"))
        {
...
AssignmentExpression: function(node, scope, compileNode)
{
    ...
    else
    {
        ...
        if (scope.isRootScope() && target.type === "Identifier" && scope.getVar(target.name) === null)
    ...
    if (!valid && scope.isLocalScope())
    {
        ...
        else if (scope.currentMethodType() === "-")
        {
            ...
            else
            {
                ...
                if (ivar)
                {
                    localVar = scope.getLocalVar(identifier);
...
MethodDeclaration: function(node, scope, compileNode)
{
    ...
    if (!duplicate)
    {
        ...
        if (method && compiler.shouldWarnAbout("parameter-types"))
            checkParameterTypes(compiler, node, nodeArguments, types, returnType, method, selector);
...
ClassStatement: function(node, scope)
{
    ...
    scope.rootScope().vars[node.id.name] = { type: "@class", node: node.id };
