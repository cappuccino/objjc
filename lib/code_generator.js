...
function checkForShadowedVars(node, scope)
{
    ...
    if (!shadowedType)
    {
        def = compiler.getClassDef(identifier);
    ...
    if (!shadowedType)
    {
        def = compiler.getProtocolDef(identifier);
    ...
    if (!shadowedType)
    {
        def = compiler.getTypeDef(identifier);
    ...
    if (!shadowedType)
    {
        def = compiler.getPredefinedGlobal(identifier);

        if (def && !def.ignoreShadow)
            shadowedType = "a predefined global";
    }

    if (shadowedType && !compiler.options.ignoreWarnings)
    {
        compiler.addWarning(
            node,
...
function variableDeclaration(node, scope, compileNode)
{
    ...
    for (let i = 0; i < node.declarations.length; i++)
    {
        ...
        else if (identifier === "self" && scope.currentObjjMethodScope())
            compiler.addError(decl.id, "'self' used as a variable name within a method");

        else if (compiler.shouldWarnAbout("shadowed-vars"))
    ...
    else // self, ivar or expression
    {
        ...
        if (receiverIsIdentifier)
        {
            ...
            if (receiver.name === "self")
                receiverMightBeNil = !receiverVar || !receiverVar.scope || receiverVar.scope.assignmentToSelf;
            else
                receiverMightBeNil = !!receiverVar || !compiler.getClassDef(receiver.name);
