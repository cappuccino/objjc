...
function functionNode(node, scope, compileNode)
{
    ...
    {
        inner.vars[param.name] = { type: "function parameter", node: param };
        compiler.checkForShadowedVars(param, scope, entityTypes.ENTITY_TYPE_FUNCTION);
    }

    if (id)
    {
        if (isDeclaration)
            scope.vars[id.name] = { type: "function", node: id };
        else
            inner.vars[id.name] = { type: "function name", node: id };
...
function objjClassDeclaration(node, scope, compileNode)
{
    ...
    if (!compiler.isUniqueDefinition(node, scope, objj.name))
        return;
...
function objjTypeStatement(node, scope, actionFunc, type)
{
    ...
    for (let i = 0; i < ids.length; i++)
    {
        ...
        if (compiler.isUniqueDefinition(node, scope, id))
        {
            if (i === 0)
                comment += `// @${type}`;
            else
