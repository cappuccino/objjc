...
var ClassDef = language.ClassDef,
    ...
    wordPrefixOperators = utils.arrayToHash([
        "delete",
        "in",
        "instanceof",
        "new",
...
function program(node, scope, compileNode)
{
    var compiler = scope.compiler,
        indentString = compiler.format.getGlobal("indent-string"),
...
{
    scope.compiler.compileDependentStatement(node.statement, scope, compileNode);
}
...
function functionStatement(node, scope, compileNode)
{
    var compiler = scope.compiler,
        buffer = compiler.jsBuffer,
    ...
    if (id)
    {
        ...
        (isDeclaration ? scope : inner).vars[name] = { type: isDeclaration ? "function" : "function name", node: id };

        if (compiler.options.transformNamedFunctionToAssignment)
        {
            buffer.concat(name, node);
    ...
    var isLambda = !isDeclaration && node.body.body.length <= 1 && Scope.parentNodeType(node) === "ObjectExpression";
...
function objectExpression(node, scope, compileNode)
{
    var properties = node.properties,
        buffer = scope.compiler.jsBuffer,
        selector = properties.length > 0 ? "left-brace" : "empty-left-brace";
...
function updateExpression(node, scope, compileNode)
{
    ...
    if (node.argument.type === "Dereference")
    {
        ...
        if (node.prefix)
            buffer.concat(node.operator + "@deref(" + node.argument.expr.name + ") */", node);
        else
        {
            buffer.concat("@deref(" + node.argument.expr.name + ")" + node.operator + " */", node);
            buffer.concatLeftParens(node, scope);
        }
...
function checkIdentifierReference(node, scope)
{
    ...
    else if (compiler.shouldWarnAbout("unknown-identifiers"))
    {
            ...
            !compiler.isPredefinedGlobal(identifier))
        {
            ...
            if (scope.receiver)
            {
                var def = compiler.findDef(identifier);
...
function arrayLiteral(node, scope, compileNode)
{
    var buffer = scope.compiler.jsBuffer;
...
function dictionaryLiteral(node, scope, compileNode)
{
    var buffer = scope.compiler.jsBuffer;
...
function importStatement(node, scope)
{
    var buffer = scope.compiler.jsBuffer,
...
function classDeclaration(node, scope, compileNode)
{
    var compiler = scope.compiler,
        buffer = compiler.jsBuffer,
...
function protocolDeclaration(node, scope, compileNode)
{
    var compiler = scope.compiler,
        buffer = compiler.jsBuffer,
    ...
    buffer.concat(ProtocolDef.declarationTemplate({ comment: comment, name: protocolName }));
...
)
{
    ...
    if (overriddenMethod && compiler.shouldWarnAbout("parameter-types"))
    {
        ...
        for (i = 1; i < declaredTypes.length; i++)
        {
            var parameterType = declaredTypes[i],
                nodeArgument = nodeArguments[i - 1];

            if (parameterType !== types[i] &&
                !(parameterType === "id" && nodeArgument.type.typeisclass))
            {
                compiler.addWarning(
                    nodeArgument.type || nodeArgument.identifier,
    ...
    {
        if (returnType.typeisclass)
            compiler.checkForUnknownType(returnType, returnType.name, classOrProtocolDef);
        ...
        for (i = 0; i < nodeArguments.length; i++)
        {
            ...
            if (type.typeisclass)
                compiler.checkForUnknownType(type, type.name, classOrProtocolDef);
        }
    }
}

function methodDeclaration(node, scope, compileNode)
{
    var compiler = scope.compiler,
    ...
    {
        for (i = 0; i < returnTypeProtocols.length; i++)
        {
...
function messageSendExpression(node, scope, compileNode)
{
    var compiler = scope.compiler,
        buffer = compiler.jsBuffer,
        useTempVar = false,
    ...
    if (node.superObject)
    {
        var superclass = scope.currentMethodType() === "+" ?
    ...
    else
    {
        ...
        var nodeBuffer = new StringBuffer(compiler);
...
function selectorLiteralExpression(node, scope)
{
    scope.compiler.jsBuffer.concat(util.format("sel_getUid(\"%s\")", node.selector), node);
}

function protocolLiteralExpression(node, scope)
{
    var buffer = scope.compiler.jsBuffer,
        name = node.id.name,
    ...
    if (protocol)
        buffer.concat(util.format("objj_getProtocol(\"%s\")", node.id.name), node);
    else
...
function dereference(node, scope, compileNode)
{
    var compiler = scope.compiler,
        buffer = compiler.jsBuffer;
...
function classStatement(node, scope)
{
    var compiler = scope.compiler,
        name = node.id.name;
...
function globalStatement(node, scope)
{
    var compiler = scope.compiler;
...
function typeDefStatement(node, scope)
{
    var compiler = scope.compiler,
...
var generators =
{
    ...
    MessageSendExpression: messageSendExpression,
    MethodDeclaration: methodDeclaration,
    NewExpression: newExpression,
    ObjectExpression: objectExpression,
    PreprocessStatement: preprocessStatement,
    Program: program,
    ProtocolDeclaration: protocolDeclaration,
    ProtocolLiteralExpression: protocolLiteralExpression,
    Reference: reference,
    ReturnStatement: returnStatement,
    SelectorLiteralExpression: selectorLiteralExpression,
