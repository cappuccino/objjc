...
function setupWarnings(/* Array */ warnings)
{
    ...
    warnings.every(function(warning)
    {
        ...
        {
            // On the first inclusive warning, reset all to false
            Object.keys(result).forEach(function(key)
            {
                result[key] = false;
            });
        ...
        {
            console.warn("objjc: " + chalk.yellow("warning: ") + chalk.gray("inclusive and exclusive compiler warnings may not be mixed"));
            return false;  // abort every()
        }
...
var Compiler = function(source, sourcePath, ast, options)
{
    this.source = source;
...
Compiler.prototype.findClassDef = function(/* String */ name)
{
    name = name.toLowerCase();

    var names = Object.keys(this.classDefs);

    for (var i = 0; i < names.length; ++i)
    {
        if (name === names[i].toLowerCase())
            return this.classDefs[names[i]];
    }
...
Compiler.prototype.checkForShadowedIvar = function(scope, identifier)
{
    ...
    var ivarRefs = scope.ivarRefs[identifier];
    ...
    if (ivarRefs)
    {
        for (var i = 0; i < ivarRefs.length; i++)
        {
            ...
            this.addNote(ivarInfo.ivar, "instance variable is declared here");
        }
...
Compiler.prototype.addGlobal = function(node, identifier)
{
    this.globals[identifier || node.name] = node;
};

Compiler.prototype.getGlobal = function(identifier)
{
    return this.globals[identifier];
};

Compiler.prototype.isGlobal = function(identifier)
{
    return identifier in this.globals;
};
...
{
    return this.predefinedGlobals[identifier] !== undefined;
};
...
Compiler.prototype.compileWithFormat = function(visitor)  // jshint ignore:line
{
    ...
    var compileNode = function(node, scope, virtualType)
    {
        ...
        if (!sameNode)
        {
            ...
            buffer.concatFormat(node, scope, "before", virtualType);
            scope.pushNode(node, virtualType);
        }
        ...
        if (!sameNode)
        {
            scope.popNode(node, virtualType);
            buffer.concatFormat(node, scope, "after", virtualType);
        }
    ...
    if (this.options.sourceMap)
        this.sourceMap = this.jsBuffer.getSourceMap();
};

Compiler.prototype.compileDependentStatement = function(node, scope, compileNode, virtualType)
{
