...
class Compiler
{
    ...
    static isIdempotentExpression(node)
    {
        let result;
        ...
        switch (node.type)
        {
            ...
            case "ArrayExpression":
                ...
                {
                    if (!this.isIdempotentExpression(element))
                        return false;
                }
            ...
            case "objj_DictionaryLiteral":
                ...
                {
                    if (!this.isIdempotentExpression(node.keys[i]))
                        return false;

                    if (!this.isIdempotentExpression(node.values[i]))
                        return false;
                }
            ...
            case "ObjectExpression":
                ...
                {
                    if (!this.isIdempotentExpression(property.value))
                        return false;
                }
            ...
            case "FunctionExpression":
                ...
                {
                    if (!this.isIdempotentExpression(param))
                        return false;
                }
            ...
            case "SequenceExpression":
                ...
                {
                    if (!this.isIdempotentExpression(expression))
                        return false;
                }
            ...
            case "UnaryExpression":
                result = this.isIdempotentExpression(node.argument);
                break;

            case "BinaryExpression":
                result = this.isIdempotentExpression(node.left) && this.isIdempotentExpression(node.right);
                break;

            case "ConditionalExpression":
                result = this.isIdempotentExpression(node.test) &&
                    this.isIdempotentExpression(node.consequent) &&
                    this.isIdempotentExpression(node.alternate);
                break;

            case "MemberExpression":
                result = this.isIdempotentExpression(node.object) &&
                    (!node.computed || this.isIdempotentExpression(node.property));
                break;

            case "objj_Dereference":
                result = this.isIdempotentExpression(node.expr);
                break;

            case "objj_Reference":
                result = this.isIdempotentExpression(node.element);
                break;
    ...
    */
    ...
    {
        if (!this.constructor.isIdempotentExpression(node))
...
`;
    ...
    */
    ...
    checkAssignment(node, scope)
    {
        ...
        {
            if (!this.isUniqueGlobalSymbol(node, scope, node))
                return;
            ...
            if (scope.isLocalVarScope())
            {
                ...
                {
                    const
                        context = scope.functionName || scope.selector,
                        issue = this.addIssue(
    ...
    checkForShadowedVars(node, scope, entityType)
    {
        ...
        if (def)
        {
            ...
            if (def.implicit)
            {
                ...
                // So we convert the original global to a local.
                def.type = "local var";
                delete def.implicit;
                shadowedType = null;
    ...
    filterIdentifierIssues(scope)
    {
        this.issues.filter(issue =>
        {
            if (issue.scope === scope &&
                (issue instanceof exceptions.UnknownIdentifierWarning ||
