...
Compiler.prototype.checkForUnknownType = function(node, type, typeIsClass, classOrProtocolDef)
{
...
Compiler.prototype.addIvars = function(node, compileNode, scope, classDef, classScope)  // -> hasAccessors
{
    ...
    for (var i = 0; i < node.ivardeclarations.length; i++)
    {
        ...
        var ivarType = ivarDecl.ivartype ? ivarDecl.ivartype.name : null,
        ...
        this.checkForUnknownType(ivarDecl, ivarType, ivarTypeIsClass, classDef);
...
Compiler.prototype.isUniqueDefinition = function(node, scope, name)
{
    ...
    if (def)
    {
        ...
        // If def is an @implementation and node is a @class (or vice versa),
        // node is not unique but it isn't an error.
    ...
    if (!previousType)
    {
        ...
        if (def)
        {
            ...
            {
                this.addError(node, "duplicate definiton of protocol '%s'", name);
