...
Compiler.prototype.initPredefinedGlobals = function()
{
    ...
    merge(this.predefinedGlobals, globals.reserved);
    merge(this.predefinedGlobals, globals.nonstandard);
    merge(this.predefinedGlobals, globals.ecmaIdentifiers);
    merge(this.predefinedGlobals, globals.newEcmaIdentifiers);
    merge(this.predefinedGlobals, globals[this.options.environment]);

    if (this.options.environment === "browser")
        merge(this.predefinedGlobals, globals.devel);
};
...
{
    last(this.issues).addHighlight(node);
};
...
Compiler.prototype.isUniqueDefinition = function(node, scope, name)
{
    ...
    if (def)
    {
        ...
        // node is not unique but it isn't an error. If node is an @implementation
        // and def is a @class, pretend the @implementation
        // is unique so it can replace the @class.
        if (startsWith(node.type, "Class"))
...
Compiler.prototype.generateAccessors = function(node, classDef) // -> accessors
{
    ...
    for (var i = 0; i < node.ivardeclarations.length; i++)
    {
        ...
        if (!selector)
            selector = "set" + capitalize(property) + ":";
...
Compiler.prototype.compileWithFormat = function(visitor)
{
