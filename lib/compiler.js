...
/* global CFURL, class_copyIvarList, class_copyMethodList, class_copyProtocolList, class_getSuperclass, method_getName, Executable, objj_getClass, protocol_copyMethodDescriptionList, protocol_getName, objj_getProtocol */
...
var Compiler = function(/* String */ source, /* String | CFURL */ url, options)  // jshint ignore:line
{
    ...
    if (this.generateWhat === CompilerGenerateCode)
        generator = visitors.codeGenerator;
    else
        generator = visitors.dependencyCollector;
...
Compiler.prototype.createClass = function(node, name, superclassName, ivars, instanceMethods, classMethods, protocols)
{
    ...
    var type = ivars ? ClassDefTypes.implementation : ClassDefTypes.class,
        classDef = this.getClassDef(name, node);

    if (classDef && type === ClassDefTypes.implementation)
        throw this.syntaxError(util.format("Reimplementation of class '%s'", name), node.classname);

    var superclass = superclassName ? this.getClassDef(superclassName, node) : null;
...
Compiler.prototype.getIvarForClass = function(/* String */ ivarName, /* Scope */ scope, node)
{
    ...
    var classDef = this.getClassDef(scope.currentClassName(), node);
...
Compiler.prototype.getClassDef = function(/* String */ name, node)
{
    if (!name)
        return null;
...
Compiler.prototype.checkProtocolConformance = function(node, classDef, protocols)
{
    ...
    if (unimplementedMethods.length > 0)
    {
        for (var i = 0; i < unimplementedMethods.length; i++)
        {
            ...
            this.addWarning(util.format("Method '%s' in protocol %s not implemented in %s", methodDef.name, protocolDef.name, node.classname.name), node.classname);
        }
    }
};
...
{
    return JSON.stringify(this.AST, null, StringBuffer.indentWidth);
};
...
function compileWithFormat(node, scope, visitor)  // jshint ignore:line
{
