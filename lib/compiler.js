...
Compiler.prototype.createClass = function(node, name, superclass, ivars, instanceMethods, classMethods, protocols)
{
    ...
    var superclassDef = superclass ? this.getClassDef(superclass.name) : null;

    if (superclass && !superclassDef)
        this.addError(
            superclass,
    ...
    return new ClassDef(node, name, superclass, ivars, instanceMethods, classMethods, protocols);
};
...
Compiler.prototype.addError = function(node)
{
    ...
    if (this.errorCount > this.options.maxErrors)
        throw new exceptions.CompileAbortedError();
...
Compiler.prototype.checkProtocolConformance = function(node, classDef, protocols)
{
    ...
    if (unimplementedMethods.length > 0)
    {
        for (var i = 0; i < unimplementedMethods.length; i++)
        {
            ...
            this.addWarning(node.classname, "method '%s' in protocol not implemented", methodDef.name);
            this.addNote(methodDef.node, "method '%s' declared here", methodDef.name);
            this.addNote(protocolDef.node.protocolname, "required for protocol '%s'", protocolDef.name);
        }
    }
};
...
Compiler.prototype.findIvar = function(name, classDef)
{
    var ivars = classDef.ivars;
...
Compiler.prototype.addIvars = function(node, compileNode, scope, classDef, classScope)  // -> hasAccessors
{
    ...
    for (var i = 0; i < node.ivardeclarations.length; i++)
    {
        ...
        if (previousDeclaration)
        {
            ...
            scope.compiler.addNote(previousDeclaration.node, "previous declaration is here");
        }
