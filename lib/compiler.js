...
Compiler.prototype.generateAccessors = function(node, classDef)  // -> accessors
{
    var buffer = this.jsBuffer,
        ...
        generatedAccessors = Object.create(null);
    ...
    for (var i = 0; i < node.ivardeclarations.length; i++)
    {
        ...
        var ivarType = ivar.ivartype ? ivar.ivartype.name : null,
            ...
            canUseDotNotation = info.property !== ivarName;
        ...
        if (!classDef.getInstanceMethod(selector))
        {
            ...
            {
                if (!generatedAccessors[ivarName])
                    generatedAccessors[ivarName] = {};

                generatedAccessors[ivarName].get = selector;
            }
        ...
        if (canUseDotNotation)
        {
            ...
            generatedAccessors[ivarName].set = selector;
        }
...
// Helper for codeGenerator.ClassDeclaration
Compiler.prototype.generateDotAccessorShims = function(node, className, accessors)
{
    var buffer = this.jsBuffer,
        properties = util.format("\n\nObject.defineProperties(%s.allocator.prototype, {", className),
        ivars = Object.keys(accessors);
    ...
    {
        var ivar = ivars[i],
            accessor = accessors[ivar];

        properties += util.format("\n\"%s\": {\n", ivar).indent();
        indent();

        if (accessor.get)
            properties += util.format("get: function() { return objj_msgSend(this, \"%s\"); }", accessor.get).indent();
        ...
        if (accessor.set)
        {
            ...
            properties += util.format(
                "set: function(newValue) { objj_msgSend(this, \"%s\", newValue); }", accessor.set
            ).indent();
        }
...
ClassDeclaration: function(node, scope, compileNode)
{
    ...
    if (haveAccessors)
        accessors = compiler.generateAccessors(node, classDef);

    if (haveMethods)
        buffer.concat("\n]);".indent());

    if (accessors)
        compiler.generateDotAccessorShims(node, className, accessors);
