...
StringBuffer.prototype.concatPrecedenceExpression = function(node, subnode, scope, compileNode, right)
{
    if (subnodeHasPrecedence(node, subnode, right))
        this.concatParenthesizedExpression(node, scope, compileNode);
    else
...
Compiler.prototype.checkCanDereference = function(scope, node)
{
    if (!isIdempotentExpression(node))
        throw this.syntaxError("Dereference of expression with side effects", node);
};
            ...
            if (argumentProtocols)
            {
                for (j = 0, size = argumentProtocols.length; j < size; j++)
                {
                    ...
                    if (!this.getProtocolDef(argumentProtocol.name))
                        this.addWarning(createMessage("Cannot find protocol declaration for '" + argumentProtocol.name + "'", argumentProtocol, this.source));
                }
            }
        }
    }
...
Compiler.prototype.checkProtocolConformance = function(node, classDef, protocols)
{
    ...
    if (unimplementedMethods && unimplementedMethods.length > 0)
    {
        for (var i = 0, size = unimplementedMethods.length; i < size; i++)
        {
            ...
            this.addWarning(createMessage("Method '" + methodDef.name + "' in protocol '" + protocolDef.name + "' is not implemented", node.classname, this.source));
        }
    }
};
...
Compiler.prototype.checkForShadowedIvar = function(scope, identifier)
{
    ...
    if (addedSelfToIvar)
    {
        ...
        for (var i = 0; i < size; i++)
        {
            ...
            atoms[dict.index] = "";
            this.addWarning(createMessage("Local declaration of '" + identifier + "' hides instance variable", dict.node, this.source));
        }
    ...
    if (scope.currentMethodType() === "-" && !scope.secondMemberExpression && !scope.isPropertyKey)
    {
        ...
        if (ivar)
        {
            ...
            else
            {
                ...
                scope.addedSelfToIvars[identifier].push({ node: node, index: buffer.length });  // jshint ignore:line
...
ArrayLiteral: function(node, scope, compileNode)
{
    ...
    buffer.concat("objj_msgSend", true);
    buffer.concatLeftParens(node, scope);
...
MethodDeclarationStatement: function(node, scope, compileNode)
{
    ...
    buffer.concat("new objj_method(sel_getUid(\"", node);
...
Reference: function(node, scope)
{
    var compiler = scope.compiler,
        buffer = compiler.jsBuffer,
        template = "function(__input)\n{^if (arguments.length)^^return %s = __input;\n\n^return %s;\n}".replace(/\^/g, indentStep);
