...
var compile = function(node, state, visitor)
{
    var compileNode = function(node, state, override)
    {
...
var compileWithFormat = function(node, state, visitor)
{
...
var checkCanDereference = function(state, node)
{
...
var parenthesizeExpression = function(generate, node, subnode, state, compileNode, right)
{
...
// Helper for codeGenerator.ClassDeclarationStatement
var addIvars = function(compiler, node, compileNode, state, classDef, className, classScope, firstIvarDeclaration, generateObjJ, saveJSBuffer)  // -> hasAccessors
{
...
var makeSelector = function(compiler, state, compileNode, nodeArguments, types, selectors, selector, generateObjJ)  // -> selector
{
...
// Helper for codeGenerator.MethodDeclarationStatement
var compileMethodBody = function(compiler, node, state, compileNode, nodeArguments, methodScope, selector, generate, generateObjJ)
{
    if (node.body)
    {
        if (!generateObjJ)
...
Identifier: function(node, state)
{
    ...
    {
        var lvar = state.getLvar(identifier, false), // Only look inside method
        ...
        if (ivar)
        {
            ...
            else
            {
                ...
                if (!state.addedSelfToIvars)
                    state.addedSelfToIvars = Object.create(null);

                if (!(identifier in state.addedSelfToIvars))
                    state.addedSelfToIvars[identifier] = [];

                state.addedSelfToIvars[identifier].push({ node: node, index: compiler.jsBuffer.length() });
...
MethodDeclarationStatement: function(node, state, compileNode)
{
    ...
    selector = makeSelector(compiler, state, compileNode, nodeArguments, types, selectors, selector, generateObjJ);
    ...
    compileMethodBody(compiler, node, state, compileNode, nodeArguments, methodScope, selector, generate, generateObjJ);
