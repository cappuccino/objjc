...
`;
    ...
    isUniqueDefinition(node, scope, name)
    {
        ...
        */
        ...
        else
        {
            ...
            if (def)
                return this.duplicateDefinitionException("@class", name, true, def.node, node);
        }
        ...
        if (!previousType)
        {
            ...
            if (def)
            {
                if (node.type === "objj_TypeDefStatement")
                    return this.duplicateDefinitionException("@typedef", name, true, def.node, node);
        ...
        if (!previousType)
        {
            ...
            if (def)
            {
                if (node.type === "objj_GlobalStatement" || (node.type === "Identifier" && scope.assignment))
                    return this.duplicateDefinitionException("@global", name, true, def.node, node);
    ...
    */
    checkIdentifierReference(node, scope)
    {
        ...
        if (scope.assignment)
            this.checkAssignment(identifier, node, scope);
        else
            this.checkForUnknownIdentifier(identifier, node, scope);
    }

    checkAssignment(identifier, node, scope)
    {
        // Assignments to properties are not checked
        if (scope.isMemberParent)
            return;

        const variable = this.getPredefinedGlobal(identifier);
        ...
        {
            if (!this.isUniqueDefinition(node, scope, identifier))
                return;
    ...
    checkForUnknownIdentifier(identifier, node, scope)
    {
