...
class Runner
{
    constructor(options)
    {
        this.options = this.constructor.setupOptions(options || {});
        this.compiler = null;
        this.destPath = null;
    ...
    static setupOptions(options)
    {
        ...
        this.initAcornOptions(Object.assign({}, options));
    ...
    generate(sourceFile, destPath)
    {
        ...
        if (this.options.output)
        {
            ...
            if (this.options.sourceMap)
                fs.writeFileSync(destPath + ".map", this.compiler.getSourceMap());
        }
    ...
    compileFileOrSource(sourcePath, source)
    {
        ...
        const acornOptions = this.options.acornOptions || {};

        if (!acornOptions.file)
        {
            if (compileFile)
                acornOptions.file = compiler.Compiler.getRelativeSourcePath(process.cwd(), sourcePath);
            else
        ...
        if (this.issues.length > 0)
        {
            ...
            {
                exitCode = this.compiler.getErrorCount() > 0 ?
        ...
        if (error && !this.options.silent)
        {
            ...
            if (this.options.stackTrace)
                console.log("\n" + (stackTrace || error.getStackTrace(Runner.STACK_FILTER)));
        }
