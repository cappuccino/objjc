...
Runner.prototype.generate = function(sourceFile, destPath, compiler)
{
    ...
    {
        if (generateCode)
            grunt.file.write(destPath, compiler.getCode());

        if (this.options.sourceMap)
            grunt.file.write(destPath + ".map", compiler.getSourceMap());
    }
...
Runner.prototype.compileFileOrSource = function(sourcePath, source)
{
    ...
    try
    {
        ...
        this.compiler = new Compiler(source, sourcePath, destPath, ast, this.options);
        ...
        if (!this.options.silent && this.compiler.getErrorCount() === 0)
            this.generate(sourcePath, destPath, this.compiler);
    }
    catch (ex)
    {
