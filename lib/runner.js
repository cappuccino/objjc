...
Runner.prototype.compileFileOrSource = function(sourcePath, source)
{
    ...
    try
    {
        ...
        compiler = new Compiler(source, sourcePath, ast, this.options);
        compiler.compileWithFormat(visitors.codeGenerator);
        issues = compiler.getIssues();

        if (issues.every(function(issue) { return !issue.isError(); }))
            this.generate(sourcePath, compiler);
    }
    catch (ex)
    {
    ...
    if (issues && issues.length > 0)
    {
        ...
        if (aborted)
            console.log(require("util").format("Compilation aborted, too many errors (>%d).", compiler.options.maxErrors));
    }
};
