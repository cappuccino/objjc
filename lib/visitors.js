...
function checkForHiddenGlobalVar(node, scope)
{
    ...
    {
        var fileVar = scope.getLocalVar(identifier);
...
function validateIdentifier(node, scope)
{
    ...
    if (!fileVar)
    {
        if (scope.assignment && compiler.shouldWarnAbout("implicit-globals"))
        {
...
BlockStatement: function(node, scope, compileNode)
{
    ...
    if (scope.isLocalScope() && scope.maxReceiverLevel)
    {
...
Identifier: function(node, scope)
{
    ...
    if (scope.isLocalScope() && !scope.secondMemberExpression && !scope.isPropertyKey)
    {
        ...
        if (!valid && !(identifier in globals.reserved))
