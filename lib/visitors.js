...
// A list of reserved JavaScript identifiers that might be used as variable names
var reservedIdentifiers = utils.arrayToHash([
        "arguments",
        "break",
        "continue",
        "debugger",
    ...
    wordPrefixOperators = utils.arrayToHash(["delete", "in", "instanceof", "new", "typeof", "void"]);
...
function checkForHiddenGlobalVar(node, scope)
{
    var compiler = scope.compiler,
        ...
        isGlobal = global.hasOwnProperty(identifier),
        classDef = compiler.getClassDef(identifier),
        fileVar = scope.getLocalVar(identifier),
        shadowType,
        shadowedNode;

    if (isGlobal || classDef || fileVar)
    {
        if (isGlobal)
        {
            shadowType = "a predefined global variable";
        }
        else if (fileVar)
        {
            var fileVarTypes = {
...
function validateIdentifier(node, scope)
{
    var compiler = scope.compiler,
        identifier = node.name,
        fileVar = scope.getLocalVar(identifier);
    ...
    if (!fileVar)
    {
        if (scope.assignment && compiler.shouldWarn("implicit-globals"))
        {
        ...
        else if (compiler.shouldWarn("unknown-identifiers"))
        {
            compiler.addIssue(
                exceptions.UnknownIdentifierWarning,
                node,
                "reference to unknown identifier '%s'",
                identifier
            );
        }
    }
...
BlockStatement: function(node, scope, compileNode)
{
    ...
    if (scope.parent && scope.maxReceiverLevel)
    {
...
TryStatement: function(node, scope, compileNode)
{
    ...
    if (node.handler)
    {
        ...
        scope.vars[name] = { type: "catch clause", node: param };
...
ForStatement: function(node, scope, compileNode)
{
    ...
    buffer.concatRightParens(node, scope);
    compiler.compileDependentStatement(body, scope, compileNode);
},
...
DebuggerStatement: function(node, scope)
{
    ...
    if (compiler.shouldWarn("debugger"))
...
VariableDeclaration: function(node, scope, compileNode)
{
    ...
    for (var i = 0; i < node.declarations.length; i++)
    {
        ...
        // If the scope has a parent, it's a local scope
        else if (scope.parent && compiler.shouldWarn("hidden-globals"))
            checkForHiddenGlobalVar(decl.id, scope);

        scope.vars[identifier] = {
            type: scope.parent ? "local var" : "file var",
            node: decl.id
        };
...
CallExpression: function(node, scope, compileNode)
{
    ...
    {
        var selfVar = scope.getLocalVar("self", true);
...
Identifier: function(node, scope)
{
    var compiler = scope.compiler,
        buffer = compiler.jsBuffer,
        identifier = node.name,
        methodType = scope.currentMethodType(),
        valid = false;

    if (scope.parent && !scope.secondMemberExpression && !scope.isPropertyKey)
    {
        if (methodType === "-")
        {
            ...
            if (ivar)
            {
                // true === only look within current scope
                var localVar = scope.getLocalVar(identifier, true);
        ...
        if (!valid && !(identifier in reservedIdentifiers))
