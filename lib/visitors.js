...
IfStatement: function(node, scope, compileNode)
{
    ...
    var single = node.consequent.type !== "BlockStatement";
    ...
    if (node.alternate)
    {
        ...
        single = node.alternate.type !== "BlockStatement";
...
ForStatement: function(node, scope, compileNode)
{
    ...
    var single = body.type !== "BlockStatement";
    ...
    if (scope.currentMethodType() === "-" && !scope.secondMemberExpression && !scope.isPropertyKey)
    {
        ...
        if (ivar)
        {
            ...
            else
            {
                ...
                scope.ivarRefs[identifier].push(
                    {
                        ivar: ivar.node,
                        node: node,
                        index: buffer.length
